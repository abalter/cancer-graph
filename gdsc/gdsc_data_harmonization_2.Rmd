...
cell_lines_website_raw = read_csv('large_files/harmonized/cell_lines.tsv')
```
...
### FTP Version
```{r}
cell_lines_ftp_raw = read.xlsx("large_files/Cell_Lines_Details.xlsx")

glimpse(cell_lines_ftp_raw)
...
cell_lines_ftp_raw %>% summarize(N = n_distinct(Sample.Name))

lapply(cell_lines_ftp_raw, n_distinct) %>% 
  data.frame() %>% 
  t() %>% 
  data.frame()
...
```{r}
print("length of ftp version")
cell_lines_ftp_raw %>% pull(Sample.Name) %>% length()
print("length of website version")
cell_lines_website %>% pull(cell_line_name) %>% length()

print("in website not ftp")
setdiff(
  cell_lines_website %>% pull(cell_line_name), 
  cell_lines_ftp_raw %>% pull(Sample.Name)
) %>% 
length()

print("in ftp not website")
setdiff(
  cell_lines_ftp_raw %>% pull(Sample.Name),
  cell_lines_website %>% pull(cell_line_name)
) %>% 
...
**IMPORTANT**
From exploring Cell Model Passports and the CCLE, I have decided that for the time being it will be best to use the "website" version and ignore the ftp version for now.
...
```{r}
# cell_lines_metadata = 
#   inner_join(cell_lines_ftp, cell_lines_website) %>% 
...
cell_lines_metadata = cell_lines_website
